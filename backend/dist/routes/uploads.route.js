...
const upload_controller_1 = require("../api/uploads/upload.controller");
...
const upload = (0, multer_1.default)({
    ...
    fileFilter: (req, file, cb) => {
        ...
        if (allowedTypes.includes(file.mimetype)) {
            ...
            cb(new Error(`File type ${file.mimetype} is not allowed. Only JPG, PNG, PDF, DOC, DOCX, and TXT files are permitted.`));
        }
    },
});
...
router.post("/", upload.single("file"), handleMulterError, upload_controller_1.uploadFile);
// Route for base64 image upload
router.post("/base64", express_1.default.json({ limit: "15mb" }), upload_controller_1.uploadBase64Image);
...
router.post("/multiple", upload.array("files", 10), // Allow up to 10 files
handleMulterError, upload_controller_1.uploadMultipleFiles);
// Route for document uploads with specific type
router.post("/document", upload.array("files", 10), handleMulterError, (req, res, next) => {
    // Force type to be 'document'
    req.query.type = req.query.type || "document";
    next();
}, upload_controller_1.uploadMultipleFiles);
// Route for application-specific uploads
router.post("/application", upload.array("files", 10), handleMulterError, (req, res, next) => {
...
router.get("/:publicId/info", upload_controller_1.getFileInfo);
